...
# -*- coding: utf-8 -*-
"""
Created on Mon Oct  9 11:18:33 2023
...
dOm_lcdm = np.gradient(Om_lcdm, zlcdm) / Om_lcdm


# plote
fig, ax = plt.subplots()
plt.tick_params(labelsize=14, color='red')
plt.plot(z, dOm_Om, color='darkgreen', label='GP Prediction', linestyle="-")
plt.plot(zlcdm, dOm_lcdm, color='red', label='$\Lambda$CDM')
plt.fill(np.concatenate([z, z[::-1]]),
         np.concatenate([dOm_Om - 1.0000 * eOm,
                        (dOm_Om + 1.0000 * eOm)[::-1]]),
         alpha=.5, fc='forestgreen', ec='None')
plt.fill(np.concatenate([z, z[::-1]]),
         np.concatenate([dOm_Om - 1.9600 * eOm,
                        (dOm_Om + 1.9600 * eOm)[::-1]]),
...
plt.legend(loc='best')
#plt.savefig('dom_om_gapp.pdf', format='pdf', bbox_inches='tight')
plt.show()
...
#np.savetxt('gamma_recon_fs8_h0.csv', np.transpose(N), delimiter=', ')
